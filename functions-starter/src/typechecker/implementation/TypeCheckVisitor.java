...
public class TypeCheckVisitor implements Visitor<Type> {
	...
	@Override
	public Type visit(FormalList fl) {
		for (int i = 0; i < fl.identifiers.size(); i++) {
			fl.types.elementAt(i).accept(this);
			fl.identifiers.elementAt(i).accept(this);
		}
		return null;
	}

	@Override
	public Type visit(ExpressionList expressions) {
		for (int i = 0; i < expressions.expressions.size(); i++){
			expressions.expressions.elementAt(i).accept(this);
		}
		return null;
	}
	...
	public Type visit(FunctionDeclaration n) {
		...
		n.body.accept(functionVisitor);
		// Check that the type of the return expression matches the declared type of the function
		Type actual = n.retValue.accept(functionVisitor);
		if (!n.type.equals(actual))
			errors.typeError(n.retValue, n.type, actual);
